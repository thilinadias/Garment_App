...
$params=[]; $where=[]; if($from){$where[]="DATE(j.created_at) >= ?"; $params[]=$from;} if($to){$where[]="DATE(j.created_at) <= ?"; $params[]=$to;}
if($factory_id){$where[]="j.factory_id = ?"; $params[]=(int)$factory_id;} if($style_id){$where[]="j.style_id = ?"; $params[]=(int)$style_id;}
$whereSql=$where?('WHERE '.implode(' AND ',$where)):'';
$sql="SELECT j.id,j.job_number,s.name AS style,j.cut_date,j.cut_qty,f.name AS factory,j.out_date,j.out_qty,j.date_count,j.status,j.created_at FROM jobs j LEFT JOIN factories f ON j.factory_id=f.id LEFT JOIN styles s ON j.style_id=s.id $whereSql ORDER BY j.created_at DESC";
$st=$pdo->prepare($sql); $st->execute($params); $rows=$st->fetchAll(); $total_out=0; foreach($rows as $r){ $total_out+=(int)$r['out_qty']; }
...
<div class="card p-3">
  ...
  <form method="get" class="row g-3 mb-3">
    ...
    <div class="col-12 d-flex gap-2">
      ...
      <a class="btn btn-primary" href="<?php echo url('reports/export_csv.php'); ?>?from=<?php echo h($from); ?>&to=<?php echo h($to); ?>&factory_id=<?php echo h($factory_id); ?>&style_id=<?php echo h($style_id); ?>">CSV</a>
